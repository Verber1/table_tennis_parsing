...
def get_dataframe():
    ...
    service = Service(executable_path=r'D:\src_git\flashscore parse\firefoxdriver\geckodriver.exe')
    driver = webdriver.Firefox(service=service)
    driver.get(url)

    time.sleep(3)

    all_rows = driver.find_elements("xpath", "//div[@class='event__match event__match--live event__match--twoLine']")
    print('Количество лайв-матчей:', len(all_rows), '\n')
